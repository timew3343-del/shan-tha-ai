...
serve(async (req) => {
  ...
  try {
    ...
    const { videoBase64, bgPreset, customBgDescription } = await req.json();
    if (!videoBase64) {
    ...
    if (costSettings?.value) {
      try { const parsed = JSON.parse(costSettings.value); creditCost = parsed.video_bg_change || 18; } catch {}
    }
    ...
    const videoFileName = `${user.id}/bg-input-${Date.now()}.mp4`;
    ...
    // Map preset to description
    const BG_DESCRIPTIONS: Record<string, string> = {
      office: "modern professional office with glass walls and city view",
      beach: "tropical beach with palm trees and turquoise water at sunset",
      city: "bustling city skyline at night with neon lights",
      nature: "lush green forest with sunlight filtering through trees",
      studio: "professional photography studio with soft lighting and gradient background",
      space: "outer space with stars, nebula and planet earth in background",
    };
    const bgDescription = customBgDescription || BG_DESCRIPTIONS[bgPreset] || BG_DESCRIPTIONS.office;

    // Create job
    const { data: job, error: jobErr } = await supabaseAdmin.from("generation_jobs").insert({
      user_id: user.id,
      tool_type: "video_bg_change",
      status: "processing",
      credits_cost: creditCost,
      credits_deducted: false,
      input_params: {
        videoUrl,
        bgPreset: bgPreset || "office",
        bgDescription,
        isAdmin,
        tool_name: "AI Video Background Changer",
      },
    }).select("id").single();
    ...
    console.log(`Video BG changer job created: ${job.id}`);

    return new Response(JSON.stringify({ status: "processing", jobId: job.id }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } });

  } catch (error: any) {
    console.error("video-bg-changer error:", error);
